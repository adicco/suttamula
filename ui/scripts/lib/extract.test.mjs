...
test('zipSegments follows pali order and joins english', () => {
...
test('zipSegments keeps pali-only segments with null english', () => {
	const pali = { 'x:1.1': 'P1', 'x:1.2': 'P2' };
	const en = { 'x:1.1': 'E1' };
	const out = zipSegments(pali, en);
	assert.deepEqual(out, [
		{ seg_id: 'x:1.1', seg_order: 0, pali: 'P1', english: 'E1' },
		{ seg_id: 'x:1.2', seg_order: 1, pali: 'P2', english: null },
	]);
});

test('zipSegments appends english-only segments after pali', () => {
	const out = zipSegments({ 'x:1.1': 'P1' }, { 'x:1.1': 'E1', 'x:0.1': 'Title' });
	assert.deepEqual(out, [
		{ seg_id: 'x:1.1', seg_order: 0, pali: 'P1', english: 'E1' },
		{ seg_id: 'x:0.1', seg_order: 1, pali: null, english: 'Title' },
	]);
});
